...
} from 'src/app/services/env.service';
...
export class ResultPage {
  ...
  vCardContact: VCardContact | undefined;
  ...
  wifiEncryption: 'NONE' | 'WEP' | 'WPA' | undefined;
  ...
  ionViewWillEnter() {
    if (this.env.detailedRecordSource != null) {
      if (
        ...
        this.env.showQrAfterCameraScan == 'on'
      ) {
        this.showQrFirst = true;
      } else if (
  ...
  async ionViewDidEnter(): Promise<void> {
    // Ensure splash is hidden when navigating directly to result (e.g. share cold start)
    await SplashScreen.hide().catch(() => {
      // Ignore
    });
    if (
      this.contentType == 'url' &&
      this.env.autoOpenUrl == 'on' &&
      this.env.recordSource == 'scan'
    ) {
      setTimeout(() => {
        this.presentToast(
          this.translate.instant('AUTO_OPEN_URL'),
          'short',
          'bottom',
        );
        if (this.isHttp) {
  ...
  setContentType(): void {
    ...
    if (tContent?.substr(0, contactPrefix.length) === contactPrefix) {
      ...
      tContent?.substr(0, emailDoconoPrefix.length) === emailDoconoPrefix
    ) {
      ...
      this.latitude = +tContent?.substring(
        geoPrefix.length,
        tContent.indexOf(','),
      );
  ...
  private isValidUrl(text: string | undefined): boolean {
    let url: URL;
  ...
  private async openWifiSettings(): Promise<void> {
    try {
      if (this.platform.is('android')) {
        // Use intent URL to jump into Wi‑Fi settings on Android.
        window.open(
      ...
      if (this.platform.is('ios')) {
        // iOS may block deep-links to Settings depending on OS version/policy.
    ...
    // Fallback: open app settings (better than doing nothing).
  ...
  get isHttp(): boolean {
    ...
    if (
      tContent.substring(0, urlPrefix1.length) === urlPrefix1 ||
      tContent.substring(0, urlPrefix2.length) === urlPrefix2
    ) {
      return true;
    }
    return false;
  ...
  async addContact(): Promise<void> {
    ...
    if (this.contentType === 'contact') {
      ...
      contactInput.name = {
        given:
          this.vCardContact?.givenName ?? this.vCardContact?.fullName ?? '',
    ...
    if (this.platform.is('ios')) {
      ...
      // Android doesn't need to get permission
  ...
  private async saveContact(contactInput: ContactInput) {
    await Contacts.createContact({ contact: contactInput })
      .then((_) => {
        this.presentToast(
          this.translate.instant('MSG.SAVED_CONTACT'),
          'short',
          'bottom',
        );
      })
      .catch((err) => {
        if (this.env.isDebugging) {
          this.presentToast(
            'Error when call Contacts.createContact: ' + JSON.stringify(err),
            'long',
            'top',
          );
        } else {
          this.presentToast(
            this.translate.instant('MSG.FAILED_SAVING_CONTACT'),
            'short',
            'bottom',
          );
        }
  ...
  async sendSms(): Promise<void> {
    if (this.smsContent && this.phoneNumber) {
      if (this.platform.is('android')) {
        await this.sms
          .send(this.phoneNumber, this.smsContent, {
          ...
          .then(
            (value) => {
              this.presentToast(
                this.translate.instant('MSG.PREPARE_SMS'),
                'short',
                'center',
              );
            },
            async (err) => {
              ...
              this.presentToast(
                this.translate.instant('MSG.FAIL_PREPARE_SMS'),
                'long',
                'center',
              );
            },
  ...
  async copyText(): Promise<void> {
    if (this.base64Decoded && this.base64Encoded) {
      ...
      await Clipboard.write({ string: this.qrCodeContent }).then(async () => {
        await this.presentToast(
          this.translate.instant('COPIED'),
          'short',
          'bottom',
  ...
  async base64(): Promise<void> {
    let failEncoded = false,
      failDecoded = false;
    try {
      this.base64EncodedText = btoa(
    ...
    if (failEncoded && failDecoded) {
      await this.presentToast(
        this.translate.instant('MSG.NOT_BASE64_EN_DE'),
        'short',
        'center',
      );
    } else if (failEncoded && !failDecoded) {
      await this.presentToast(
        this.translate.instant('MSG.NOT_BASE64_EN'),
        'short',
        'center',
      );
    } else if (!failEncoded && failDecoded) {
      await this.presentToast(
        this.translate.instant('MSG.NOT_BASE64_DE'),
        'short',
        'center',
      );
    }
  ...
  generateVCardContact(): void {
    ...
    if (!lines[1] || (lines[1] && lines[1] !== 'VERSION:3.0')) {
      this.presentToast(
        this.translate.instant('MSG.ONLY_VCARD_3_0'),
        'short',
        'center',
      );
      return;
    ...
    lines.forEach((line) => {
      ...
      if (tLine.toUpperCase().substr(0, fullNameId1.length) === fullNameId1) {
        ...
        tLine.toUpperCase().substr(0, fullNameId2.length) === fullNameId2
      ) {
      ...
      if (tLine.toUpperCase().substr(0, workEmailId1.length) === workEmailId1) {
        ...
        tLine.toUpperCase().substr(0, workEmailId2.length) === workEmailId2
      ) {
        this.vCardContact!.workEmail = tLine.substr(workEmailId2.length);
      } else if (
        tLine.toUpperCase().substr(0, workEmailId3.length) === workEmailId3
      ) {
      ...
      if (tLine.toUpperCase().substr(0, homeEmailId1.length) === homeEmailId1) {
        ...
        tLine.toUpperCase().substr(0, homeEmailId2.length) === homeEmailId2
      ) {
        this.vCardContact!.homeEmail = tLine.substr(homeEmailId2.length);
      } else if (
        tLine.toUpperCase().substr(0, homeEmailId3.length) === homeEmailId3
      ) {
        this.vCardContact!.homeEmail = tLine.substr(homeEmailId3.length);
      }
      if (
        tLine.toUpperCase().substr(0, defaultEmailId1.length) ===
        defaultEmailId1
      ) {
        this.vCardContact!.defaultEmail = tLine.substr(defaultEmailId1.length);
      } else if (
        tLine.toUpperCase().substr(0, defaultEmailId2.length) ===
        defaultEmailId2
      ) {
        this.vCardContact!.defaultEmail = tLine.substr(defaultEmailId2.length);
      } else if (
        tLine.toUpperCase().substr(0, defaultEmailId3.length) ===
        defaultEmailId3
      ) {
        this.vCardContact!.defaultEmail = tLine.substr(defaultEmailId3.length);
      }
      if (
        tLine.toUpperCase().substr(0, workPhoneNumberId1.length) ===
        workPhoneNumberId1
      ) {
        this.vCardContact!.workPhoneNumber = tLine.substr(
          workPhoneNumberId1.length,
        );
      } else if (
        tLine.toUpperCase().substr(0, workPhoneNumberId2.length) ===
        workPhoneNumberId2
      ) {
        this.vCardContact!.workPhoneNumber = tLine.substr(
          workPhoneNumberId2.length,
        );
      } else if (
        tLine.toUpperCase().substr(0, workPhoneNumberId3.length) ===
        workPhoneNumberId3
      ) {
        this.vCardContact!.homePhoneNumber = tLine.substr(
          workPhoneNumberId3.length,
        );
      }
      if (
        tLine.toUpperCase().substr(0, homePhoneNumberId1.length) ===
        homePhoneNumberId1
      ) {
        this.vCardContact!.homePhoneNumber = tLine.substr(
          homePhoneNumberId1.length,
        );
      } else if (
        tLine.toUpperCase().substr(0, homePhoneNumberId2.length) ===
        homePhoneNumberId2
      ) {
        this.vCardContact!.homePhoneNumber = tLine.substr(
          homePhoneNumberId2.length,
        );
      } else if (
        tLine.toUpperCase().substr(0, homePhoneNumberId3.length) ===
        homePhoneNumberId3
      ) {
        this.vCardContact!.homePhoneNumber = tLine.substr(
          homePhoneNumberId3.length,
        );
      }
      if (
        tLine.toUpperCase().substr(0, mobilePhoneNumberId1.length) ===
        mobilePhoneNumberId1
      ) {
        this.vCardContact!.mobilePhoneNumber = tLine.substr(
          mobilePhoneNumberId1.length,
        );
      } else if (
        tLine.toUpperCase().substr(0, mobilePhoneNumberId2.length) ===
        mobilePhoneNumberId2
      ) {
        this.vCardContact!.mobilePhoneNumber = tLine.substr(
          mobilePhoneNumberId2.length,
        );
      }
      if (
        tLine.toUpperCase().substr(0, defaultPhoneNumberId.length) ===
        defaultPhoneNumberId
      ) {
        this.vCardContact!.defaultPhoneNumber = tLine.substr(
          defaultPhoneNumberId.length,
        );
      }
    });
  }
  ...
  prepareMailToEmail(): void {
    ...
    if (emailParts.length > 1) {
      ...
      secondEmailParts.forEach((part) => {
        ...
        if (
          part.toLowerCase().substr(0, subjectPrefix.length) === subjectPrefix
        ) {
          this.emailSubject = decodeURIComponent(
            part.substr(subjectPrefix.length),
          );
        }
  ...
  prepareMATMSGEmail() {
    ...
    if (emailParts.length > 0) {
      ...
      emailParts.forEach((part) => {
        ...
        if (
          part.toUpperCase().substr(0, subjectPrefix.length) === subjectPrefix
        ) {
          this.emailSubject = decodeURIComponent(
            part.substr(subjectPrefix.length),
          );
  ...
  prepareWifi(): void {
    ...
    if (wifiParts.length > 0) {
      ...
      wifiParts.forEach((part) => {
        if (
          part.toUpperCase().substr(0, encryptionPrefix.length) ===
          encryptionPrefix
        ) {
          const method = part.substr(encryptionPrefix.length) as
            | 'WPA'
            | 'WEP'
            | 'nopass';
          this.wifiEncryption =
            method === 'nopass' ? 'NONE' : method === 'WPA' ? 'WPA' : 'WEP';
        }
        if (part.toUpperCase().substr(0, ssidPrefix.length) === ssidPrefix) {
          this.wifiSSID = part
            .substr(ssidPrefix.length)
            .replace('ä', ';')
            .replace('Ä', ':');
        }
        if (
          part.toUpperCase().substr(0, passwordPrefix.length) === passwordPrefix
        ) {
          this.wifiPassword = part
            .substr(passwordPrefix.length)
            .replace('ä', ';')
            .replace('Ä', ':');
        }
        if (
          part.toUpperCase().substr(0, hiddenPrefix.length) === hiddenPrefix
        ) {
          this.wifiHidden =
            part.substr(hiddenPrefix.length).toLowerCase() === 'true'
              ? true
              : false;
        }
      });
  ...
  get isValidEan(): boolean {
    ...
    const isValidLength =
      this.qrCodeContent.length === 18 ||
      this.qrCodeContent.length === 14 ||
      this.qrCodeContent.length === 13 ||
      this.qrCodeContent.length === 8 ||
      this.qrCodeContent.length === 5;
    return (
      isValidLength &&
      /^\d+$/.test(this.qrCodeContent) &&
